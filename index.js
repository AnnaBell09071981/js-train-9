...
function findElementGreaterThan(arr, num) {
  if(!Array.isArray(arr)) {
    ...
    const arrfind = arr.find(element => element > num);
    if(arrfind < arr[num]) {
...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = (x => x.name === name);
...
function sumAndPrint(arr) {
  ...
  const arrForEach = arr.forEach(element => console.log(element));
  return sum;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const filteredSquaredArray = arr2.filter((item) => (item) > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = arr1.reduceRight((accumulator, currentValue) => accumulator + currentValue);
  const arrForEach = arr1.forEach(element => console.log(element));
  return sum;
...
function wordLengths(arr) {
  ...
  const newArr = arr.map(arr => arr.length);
  return newArr;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
...
function capitalizeWords(arr) {
  ...
  const capitalizedArray = arr.map((arr) => arr.charAt(0).toUpperCase() + arr.slice(1));
...
function calculateTotalPrice(arr) {
  ...
  // const arr1 = arr.flat();
  // console.log(arr1);
  const totalPrice = arr.reduce( function (sum, currentAccount) {
